...
public class TransactionRepository implements TransactionRepositoryType
{
    ...
    {
        final Web3j web3j = getWeb3jService(chainId, EthTxnNetwork.values()[preferenceRepository.getSelectedPrivateEthNetwork()]);
    ...
    private BigInteger gasPriceForNode(long chainId, BigInteger gasPrice)
    {
        if (EthereumNetworkRepository.hasGasOverride(chainId))
            return EthereumNetworkRepository.gasOverrideValue(chainId);
    ...
    {
        EthTxnNetwork txnNetwork = EthTxnNetwork.values()[preferenceRepository.getSelectedPrivateEthNetwork()];

        Timber.d("txnNetwork: %s", txnNetwork.toString());
        return Single.fromCallable(() -> {
            ...
            inDiskCache.putTransaction(from, newTx, txnNetwork);
